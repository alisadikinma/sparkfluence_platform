...
- **After modifying code, run `graphify update .`** (AST-only, no API cost) so the graph doesn't go stale. Refresh the global brain: `graphify global add graphify-out/graph.json --as sparkfluence_platform`.
...
- **Hook selection flow:** After analysis, GenerateStep calls `generate-carousel-images` with `action: 'suggest_hooks'` → returns 3 options (PRIMARY/SECONDARY/WILDCARD) based on `contentCategory` from vision analysis. User selects before generation. Selected `hook_category` + `visual_action` passed to generation call. `suggestHookOptions()` uses topic→hook category mapping (15 categories) with visual action pairing + psychology descriptions.
- **Language setting:** SourceStep has Primary (headline) + Subtitle (secondary) language selector. 3 languages: Indonesian (ID), English (EN), Hindi (HI). Subtitle can be "None" for monolingual. Saved to `carousel_projects.settings.language` AND `localStorage` (`sparkfluence_carousel_language`) for cross-project defaults — next project auto-uses last selection. Passed to `generate-carousel-images` as `language_settings` → enforces bilingual text overlay in P4 prompt (e.g., Indonesian headline + English subtitle like "RAHASIA GILA" + "The Insane Secret Behind..."). Types: `CarouselLanguage`, `CarouselLanguageSettings`, `CAROUSEL_LANGUAGES` in `types/carousel.ts`.
- **Reload title button:** RefreshCw icon in CarouselWorkspace header, next to pencil edit. Calls `analyze-carousel-source` with HOOK slide image only → multimodal vision extracts topic → updates `carousel_projects.title`. Checks auth session before calling edge function. Shows Loader2 spinner during re-detection.
- **Title auto-update:** GenerateStep ALWAYS updates title from vision-analyzed topic after `handleAnalyze()` completes (not just when "Untitled"). SourceStep derives title from validation claims which often don't match the actual visual headline — GenerateStep's multimodal vision override is more accurate.
- **Image persistence:** Generated images downloaded from fal.ai CDN → uploaded to `carousel-images` Supabase Storage bucket via `persistImageToStorage()`. Falls back to fal.ai CDN URL on upload failure (with console warning). File path: `{projectId}/slide-{index}-{timestamp}.png`, `upsert: true`.
- **AI Decision badges:** After generation, `carousel_slides.analysis_data.autoDecisions` stores hookCategory, visualAction, cameraVariant, emotionalArc (intensity + beat), wowScore, contentCategory, foreshadowType, ctaType. GenerateStep renders colored badges on each generated slide card: hook category (emerald), visual action (cyan), emotional arc (amber), WOW score (emerald/amber/red by threshold).
- **WOW gate enforcement:** `scoreWOWGate()` scores 8 cinematic elements (lighting, depth, atmosphere, color, emotion, camera, texture, cinematic ref). Enforces 6/8 minimum — if < 6, retries once with `getMissingWOWElements()` feedback via `callLLM()`. Proceeds with warning if still < 6 after retry.
- **Emotional arc in prompts:** `mapEmotionalArc(slideIndex, totalSlides, slideType)` maps each slide position to intensity (1-6) and beat (HIGH/DIP/BUILD/MINI-HOOK/CLIMAX/WARM). Injected into user prompt as `EMOTIONAL ARC: [beat] ([intensity]/6)`. HOOK=6/6, FORE=3/6, CTA=1/6, BODY=position-based (2-5/6), mini-hook at 55-75% through.
...
| GenerateStep keeps refreshing / infinite loop | `CarouselWorkspace.fetchProject()` was setting `loading=true` on every call → GenerateStep unmounts → remounts → auto-analyze → title update → onProjectUpdate → loop. Fixed: `fetchProject()` no longer sets `loading=true` on subsequent calls. |
