...
const TOPICS_CACHE_KEY = 'sparkfluence_scriptlab_topics_v5';
...
export const TopicRecommendations: React.FC<TopicRecommendationsProps> = ({
  ...
  // TikTok Challenge state
  const [tiktokChallenges, setTiktokChallenges] = useState<TikTokChallenge[]>([]);
  const [selectedChallenge, setSelectedChallenge] = useState<TikTokChallenge | null>(null);

  // Horizontal scroll on mouse wheel for trending chips (must be non-passive to preventDefault)
  useEffect(() => {
  ...
  // Horizontal scroll on mouse wheel for challenge chips
  useEffect(() => {
    const el = challengeScrollRef.current;
    if (!el) return;
    const handler = (e: WheelEvent) => {
      if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
        e.preventDefault();
  ...
  // Fetch TikTok challenges on mount
  useEffect(() => {
    const fetchChallenges = async () => {
      try {
        ...
        if (error || !data) {
          console.log('[TopicRecommendations] No TikTok challenges:', error?.message);
          return;
        }
        setTiktokChallenges(data as TikTokChallenge[]);
      } catch (err) {
        console.error('[TopicRecommendations] Error fetching challenges:', err);
      }
    };
    fetchChallenges();
  }, []);

  // Handle challenge selection (toggle on/off)
  const handleSelectChallenge = (challenge: TikTokChallenge) => {
    if (disabled) return;
    const next = selectedChallenge?.id === challenge.id ? null : challenge;
    setSelectedChallenge(next);
    onSelectChallenge?.(next);
  };
      ...
      <div className="mb-6 sm:mb-8 w-full">
        ...
        <div className="hidden sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {Array.from({ length: 10 }).map((_, i) => (
            <div
              key={i}
              className="h-28 bg-[#12121a] border border-[#262626] rounded-xl animate-pulse flex items-center justify-center"
            >
    ...
    <div className="mb-6 sm:mb-8 w-full">
      ...
      <div className="mb-3 sm:mb-4" ref={searchContainerRef}>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-2.5 sm:py-3 bg-[#12121a] border border-[#262626] rounded-xl text-sm text-[#FAFAF9] placeholder-[#57534E] focus:border-emerald-500/50 focus:outline-none transition-colors"
        ...
        {/* Trending Chips — single-line horizontal scroll */}
          ...
          <div className="relative mt-3">
            ...
            >
              <div className="flex items-center gap-2 w-max">
                <span className="text-[#78716C] text-xs whitespace-nowrap font-medium">{uiText.trendingNow}</span>
                {trendingChips.map((chip, i) => {
                  const isTikTok = chip.source === 'tiktok';
                  const sourceColor = isTikTok ? 'bg-pink-500' : chip.source === 'google' ? 'bg-blue-500' : chip.source === 'youtube' ? 'bg-red-500' : chip.source === 'news' ? 'bg-emerald-500' : 'bg-amber-500';
                  const hoverBorder = isTikTok ? 'hover:border-pink-500/40 hover:text-pink-400' : 'hover:border-emerald-500/40 hover:text-emerald-400';
                  return (
                    <button
                      key={i}
                      onClick={() => handleKeywordSearch(chip.keyword)}
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#12121a] border border-[#262626] rounded-full text-xs text-[#A8A29E] ${hoverBorder} transition-colors whitespace-nowrap`}
                    >
            ...
            {/* Right fade hint */}
            <div className="absolute right-0 top-0 bottom-2 w-12 bg-gradient-to-l from-[#0B0E14] to-transparent pointer-events-none" />
          </div>
        )}

        {/* TikTok Challenge Chips — separate row, pink themed */}
        {tiktokChallenges.length > 0 && (
          <div className="relative mt-3">
            <div
            ...
            >
              <div className="flex items-center gap-2 w-max">
                <span className="text-pink-400/70 text-xs whitespace-nowrap font-medium flex items-center gap-1">
                  <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-current" xmlns="http://www.w3.org/2000/svg">
                    <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.88-2.88 2.89 2.89 0 0 1 2.88-2.88c.28 0 .56.04.82.1v-3.5a6.37 6.37 0 0 0-.82-.05A6.34 6.34 0 0 0 3.15 15.2a6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.34-6.34V8.71a8.36 8.36 0 0 0 4.76 1.49V6.77a4.83 4.83 0 0 1-1-.08z" />
                  </svg>
                  {language === 'id' ? 'Challenge:' : 'Challenges:'}
                </span>
                {tiktokChallenges.map((ch) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={disabled}
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all whitespace-nowrap ${
                        isActive
                          ? 'bg-pink-500/20 border border-pink-500/50 text-pink-300 shadow-sm shadow-pink-500/10'
                          : 'bg-[#12121a] border border-[#262626] text-[#A8A29E] hover:border-pink-500/40 hover:text-pink-400'
                      }`}
        ...
        {/* Selected Challenge Badge + Guidance Hint */}
        {selectedChallenge && (
          <div className="mt-2 space-y-1.5">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-pink-500/10 border border-pink-500/30 rounded-lg text-xs">
                <span className="text-pink-400 font-medium">{selectedChallenge.name}</span>
                <span className="text-[#78716C]">—</span>
                <span className="text-[#A8A29E] max-w-[250px] truncate">{selectedChallenge.description}</span>
                <button
                  onClick={() => { setSelectedChallenge(null); onSelectChallenge?.(null); }}
                  className="ml-1 text-[#78716C] hover:text-pink-400 transition-colors"
                >
                  <X className="w-3 h-3" />
                </button>
              </span>
            </div>
            <p className="text-[#78716C] text-xs animate-pulse">
              {language === 'id'
                ? '↓ Sekarang pilih topik di bawah — challenge akan jadi format/style video lo'
                : language === 'hi'
                ? '↓ अब नीचे एक विषय चुनें — चैलेंज आपके वीडियो का फॉर्मेट होगा'
                : '↓ Now pick a topic below — the challenge will be your video format/style'}
            </p>
          </div>
        )}
      </div>
      ...
      {searchLoading && (
        <>
          ...
          {/* Desktop skeleton */}
          <div className="hidden sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-4">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={`search-skeleton-${i}`} className="h-28 bg-[#12121a] border border-[#262626] rounded-xl animate-pulse flex items-center justify-center">
                <div className="w-5 h-5 border-2 border-primary/30 border-t-primary rounded-full animate-spin" />
              </div>
            ))}
          </div>
        ...
        >
          ...
          >
            {[...topics].sort((a, b) => {
              ...
              const isTikTok = topic.trending_source === 'tiktok';
              return (
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${isSelected
                        ...
                        : isTikTok
                          ? "bg-[#12121a] border-pink-500/20"
                          : "bg-[#12121a] border-[#262626]"
                      }
                    `}
                  >
                    {/* Source badge + check */}
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-1.5">
                        {badge && (
                          <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${badge.bg} ${badge.border} ${badge.text}`}>
                            {badge.label}
                          </span>
                        )}
                          </div>
                    ...
                    <h4 className="text-[#FAFAF9] font-semibold text-sm mb-1.5 line-clamp-2 leading-snug">
                      {topic.title}
                    </h4>
                    ...
                    {topic.hashtags && topic.hashtags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {topic.hashtags.map((tag, i) => (
                          <span key={i} className="text-[10px] text-emerald-400/70 bg-emerald-500/10 px-1.5 py-0.5 rounded flex items-center gap-0.5">
          ...
          {[...topics].sort((a, b) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                className={`
                  text-left p-3.5 rounded-xl border transition-all duration-200 flex flex-col
                  hover:scale-[1.01] active:scale-[0.99]
                  ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}
                  ${isSelected
                    ? "bg-emerald-500/10 border-emerald-500/50 shadow-md shadow-emerald-500/10 ring-1 ring-emerald-500/30"
                    : isTikTok
                      ? "bg-[#12121a] border-pink-500/20 hover:border-pink-500/40 hover:bg-pink-500/5"
                      : "bg-[#12121a] border-[#262626] hover:border-[#3f3f46] hover:bg-[#161616]"
                  }
                `}
              >
                <div className="flex items-center justify-between mb-2">
                ...
                <h4 className="text-[#FAFAF9] font-medium text-sm mb-1.5 line-clamp-2 leading-snug">
                  {topic.title}
                </h4>
                <p className="text-[#A8A29E] text-xs line-clamp-2 leading-relaxed flex-1">
                  {topic.description}
                </p>
                {topic.hashtags && topic.hashtags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
        ...
        {loadingMore && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mt-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={`skeleton-${i}`} className="p-3.5 rounded-xl border border-[#262626] bg-[#12121a] animate-pulse flex flex-col gap-2">
                <div className="w-14 h-4 bg-[#161616] rounded-full" />
                <div className="w-full h-4 bg-[#161616] rounded" />
                <div className="w-3/4 h-3 bg-[#161616] rounded" />
              </div>
            ))}
          </div>
        )}
